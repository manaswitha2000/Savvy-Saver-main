...
def update_basket(request, slug):
    ...
    start_total_jiomart = 0.00
    start_total_flipkart = 0.00
    start_total_amazon = 0.00

    for item in basket.basketitem_set.all():
        product_total_jiomart = float(item.product.price_jiomart) * item.quantity
        ...
        start_total_jiomart += product_total_jiomart
    ...
    basket.total_jiomart = start_total_jiomart
